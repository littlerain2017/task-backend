...
REF_CANON_BLOCK = """
这本书的设定（{canon_name}，只用来让你知道整本在做什么，**不要拿它当场次评**）：
<背景>
{canon}
</背景>
"""
...
注意：**参考要针对这几场的难题，不是针对整个项目的题材。**有背景时，用它判断这几场
在整体里承担什么——但举的作品必须对准这里正在解决的问题。

记住：难题匹配 > 题材匹配。每部必须说清它**具体**怎么破的——落到一场戏：谁在场、
做了什么、镜头看着哪里。把握不准的标【凭印象】，不确定的直接别写。
最后一句说哪部的哪场戏最值得先看。
"""
...
{question}
{text_block}
按你的「卡住了」流程回答。**这一档不是来推荐片子的，她要的是出路。**

1. 先判断她卡的是哪一类（进出点 / 内心落不到动作上 / 对白太直白没潜台词 /
   信息一交代就假 / 觉得俗 / 这场该不该存在），把判断说出来。
2. **给 2-3 个具体做法，这是回答的主体，篇幅压在这里。**每个做法说清楚具体干什么
   （改成什么动作、换什么位置、加什么道具、删掉哪句、从哪一秒切入），再用一句话说
   为什么这样能解开。做法之间要互相不同，别是同一个思路的三种说法。
3. 作品在这一档只是脚注——需要时附一句「（《XX》那场戏就是这么干的）」即可，
   **不要用「找参考」那套每部三条的格式，也不要为了凑数硬塞作品。**

如果这场的问题是它根本不该存在，直接说，别给做法。
不要替她写成品台词或整段动作行——给做法和示意可以，交一稿让她照抄不行。
"""
